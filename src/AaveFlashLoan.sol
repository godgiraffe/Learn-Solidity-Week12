...
contract AaveFlashLoan is IFlashLoanSimpleReceiver {
    ...
    function executeOperation(
        ...
        CErc20Delegator(payable(cTokenCollateral)).approve(cTokenCollateral, CTokenInterface(cTokenCollateral).balanceOf(address(this)));
        CErc20Delegator(payable(cTokenCollateral)).redeem(CTokenInterface(cTokenCollateral).balanceOf(address(this)));

        // 把 UNI swap to USDC, 才能還 aave 錢
        // uint uniAmount = IERC20(UNI_ADDRESS).balanceOf(address(this));
        IERC20(UNI_ADDRESS).approve(address(swapRouter), IERC20(UNI_ADDRESS).balanceOf(address(this)));
        ...
        // 把利潤打回給 liquidator，剩下的不能打， aava 那邊會用 transferFrom 還錢
        IERC20(asset).transfer(liquidator, profit);
        return true;
    }
